...
import 'internet_provider_beeline.dart';
// ignore: must_be_immutable
class InternetPageBeeline extends StatefulWidget {
  static const String id="internet_page_beeline";
  late Color col;
  InternetPageBeeline({required this.col});
  static Widget screen(Color col)=>ChangeNotifierProvider<InternetProviderBeeline>(
    create:(context)=>InternetProviderBeeline(),
    child: InternetPageBeeline(col: col,),
  );
  @override
  _InternetPageBeelineState createState() => _InternetPageBeelineState();
}

class _InternetPageBeelineState extends State<InternetPageBeeline> with AddMessText,InfoShow{
  ComInter? comIn;
  List? month=[],week=[],day=[];
  void getInfoInternetPack()async{
    ...
    comIn?.list.forEach((i){
      if(i.operator==4&&i.muddat.split(" ").sorted()[0]=="30"&&i.category==18){
        month?.add(InternetPackages(mb:"${i.hajmi.split("+")[0]}",about: "To`plam narxi::${i.price}\nBerilgan trafik hajmi::${i.hajmi}\nAmal qilish muddati::${i.muddat}",desc:"${i.description}"));
      }else if(i.operator==4&&i.category==19){
        week?.add(InternetPackages(mb:"${i.hajmi}",about: "To`plam narxi::${i.price}\nBerilgan trafik hajmi::${i.hajmi}\nAmal qilish muddati::${i.muddat}",desc:"${i.description}"));
      }else if(i.operator==4&&i.category==20){
        day?.add(InternetPackages(mb:"${i.hajmi.split("+")[0]}",about: "To`plam narxi::${i.price}\nBerilgan trafik hajmi::${i.hajmi}\nAmal qilish muddati::${i.muddat}",desc:"${i.description}"));
      }
    });
  }
  ...
  List? lt=[
    ...
    "Haftalik internet paketlar",
    "Kunlik internet paketlar",
  ];
  String t='Beeline kompaniyasi internet paketlari';
  String content='Info';
  @override
 void initState(){
    super.initState();
  ...
  Widget build(BuildContext context){
    ...
    return Consumer<InternetProviderBeeline>(
