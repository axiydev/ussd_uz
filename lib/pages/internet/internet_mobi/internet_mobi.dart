...
import 'internet_provider_mobi.dart';
// ignore: must_be_immutable
class InternetPageMobi extends StatefulWidget {
  static const String id="internet_page_mobi";
  late Color col;
  InternetPageMobi({required this.col});
  static Widget screen(Color col)=>ChangeNotifierProvider<InternetProviderMobi>(
    create:(context)=>InternetProviderMobi(),
    child: InternetPageMobi(col: col,),
  );
  @override
  _InternetPageMobiState createState() => _InternetPageMobiState();
}

class _InternetPageMobiState extends State<InternetPageMobi> with AddMessText,InfoShow{
  ComInter? comIn;
  List? month=[],night=[],tungi_drive=[],day=[],onnet=[];
  void getInfoInternetPack()async{
    comIn=HiveDB.loadInterInfo();
    print(comIn?.list.first.description);
    comIn?.list.forEach((i){
      if(i.operator==1&&i.muddat.split(" ").sorted()[0]=="30"&&i.category==4){
      ...
      }else if(i.operator==1&&i.category==5){
        night?.add(InternetPackages(mb:"${i.hajmi}",about: "To`plam narxi::${i.price}\nBerilgan trafik hajmi::${i.hajmi}\nAmal qilish muddati::${i.muddat}",desc:"${i.description}"));
      }else if(i.operator==1&&i.category==6){
        tungi_drive?.add(InternetPackages(mb:"${i.hajmi}",about: "To`plam narxi::${i.price}\nBerilgan trafik hajmi::${i.hajmi}\nAmal qilish muddati::${i.muddat}",desc:"${i.description}"));
      }else if(i.operator==1&&i.category==16){
        day?.add(InternetPackages(mb:"${i.hajmi}",about: "To`plam narxi::${i.price}\nBerilgan trafik hajmi::${i.hajmi}\nAmal qilish muddati::${i.muddat}",desc:"${i.description}"));
      }else if(i.operator==1&&i.category==17){
        onnet?.add(InternetPackages(mb:"${i.hajmi}",about: "To`plam narxi::${i.price}\nBerilgan trafik hajmi::${i.hajmi}\nAmal qilish muddati::${i.muddat}",desc:"${i.description}"));
      }
    });
  }
  ...
  String t='Mobiuz kompaniyasi internet paketlari';
  String content='Info';
  @override
 void initState(){
    super.initState();
  ...
  Widget build(BuildContext context){
    ...
    return Consumer<InternetProviderMobi>(
                  ...
                  PageView(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        color: Colors.white,
                        child: ListView.builder(
                          itemCount:tungi_drive?.length,
                          shrinkWrap: true,
                          physics: AlwaysScrollableScrollPhysics(),
                          itemBuilder:(context,index)=>GestureDetector(child: myColumnWid(context,package: tungi_drive![index],color: widget.col),onTap: ()=>showText(context,package: tungi_drive![index],otherButton: 'Aktivlashtirish' ),),
                        ),
                      ),
                      Container(
                        margin: EdgeInsets.only(top:size.width*0.23),
                      ...
                      Container(
                        ...
                        color: Colors.white,
                        child: ListView.builder(
                          itemCount:onnet?.length,
                          shrinkWrap: true,
                          physics: AlwaysScrollableScrollPhysics(),
                          itemBuilder:(context,index)=>GestureDetector(child: myColumnWid(context,package: onnet![index],color: widget.col),onTap: ()=>showText(context,package: onnet![index],otherButton: 'Aktivlashtirish' ),),
                        ),
                      ),
                    ],
                  ),
